...
def source_to_glb_vertices(vertices: np.ndarray) -> tuple[np.ndarray, bool]:
    return np.column_stack([vertices[:, 0], vertices[:, 2], vertices[:, 1]]), False
...
def build_track_mesh_glb(
    ...
    *,
    ...
    v, flip_winding = source_to_glb_vertices(v)
    if flip_winding:
