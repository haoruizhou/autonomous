...
export default function App() {
  ...
  useEffect(() => {
    if (!track) return;
    setEditedCones(track.raw.cones);
  ...
  const addCone = useCallback((xyz: [number, number, number], type: 'boundary' | 'direction') => {
    const id = nextConeId.current++;
    setEditedCones((prev) => [
      ...prev,
      { id, xyz, n_observations: 1, mean_conf: 1.0, cone_type: type, knocked_over: false },
    ]);
  }, []);
  ...
  const toggleDown = useCallback((id: number) => {
    setEditedCones((prev) =>
      prev.map((c) => (c.id === id ? { ...c, knocked_over: !(c.knocked_over ?? false) } : c)),
    );
  }, []);
  ...
  const exportTrack = useCallback(() => {
    if (!track) return;
    const exported = { ...track.raw, cones: editedCones };
  ...
  const knockedCount = editedCones.filter((c) => c.knocked_over).length;
